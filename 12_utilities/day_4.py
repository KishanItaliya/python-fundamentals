...
# Using locale for region-specific formatting
import locale
def format_number(number):
    return locale.format_string("%d", number, grouping=True)

# Direct formatting without a separate function
...
def calculate_minutes(age_years):
    ...
    return format_number(round(total_days)), format_number(round(total_hours)), format_number(round(total_minutes))
